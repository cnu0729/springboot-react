...
public class PaymentController {
	//application.properties에 설정 키 이름을 가져오기 위해 value
	...
	//위젯 -> 페이먼트 브랜드페이 결제랑 결제방식이 살짝 달라서 widget
	@PostMapping("/widget")
	...
	//payment라는 주소로 결제정보가 들어오면 결제확인 창구로 넘겨주는 것 결제정보와 결제하고자 하는 사용자의 비밀번호
	@PostMapping("/payment")
	...
	public ResponseEntity<?> confirmBrandpay(@RequestBody Map<String, String> requestBody) {
		return confirmBrandpay(requestBody, encodeSecretKey(apiSecretKey));
	}
	...
	private ResponseEntity<?> confirmBrandpay(Map<String, String> requestBody, String encodedKey){
		//  fetch("https://api.tosspayments.com/v1/payments/confirm", {
		...
		return new ResponseEntity<>(response.getBody(), response.getStatusCode());
			...
			// 사용자한테 보내는 응답 상태메세지 잘못된 요청으로 안됐어. 상태코드 보낸 것
			return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST)encodedKey;
		}
	}
}
...
/*
  ...
  HttpEntity : HTTP 요청 또는 응답의 본문(body)와 헤더(headers)를 포함하는 객체
  	...
  	└ 헤더(header): HTTP 헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지 .. 어떤 파일이 들어오는 것인가? 누가 보내나?
  HttpEntity<문자열이면 문자열, 숫자면 숫자, 모르겠으면 비워두기> abc = new HttpEntity<비워져있어도 무관>("요청 본
  
  ResponseEntity (Response = 응답 / Http를 상속받아서 Http 기능에 응답에 대한 기능을 추가로 설정한 Entity)
  : 사용자가 요청한 응답을 개발자가 다시 사용자한테 전달할 때 사용
  ResponseEntity<String 이면 string, Integer면 Integer, 여러값이면 ?, 모르겠으면 비워두기?> = new ResponseEntity<>("응답본문", headers);
  ...
  RequestEntity<String이면 String, Integer면 Integer, 여러값이면 ?, 모르겠으면 비워두기> = new RequestEntity<>("요청본문",headers);
 ...
 ResponseEntity	  HttpEntity상속         HTTP 응답 반환		           상태 코드 없음(성공여부)
