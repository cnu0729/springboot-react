...
/* 
 백엔드에서 가져온 값 문자열 처리  (네이버에서 가져온 값, 특정값을 변환, 두칸 들여쓰기)
{       JSON .         stringify( userInfo          ,   null      ,      2      ) }
 JSON.stringfy = 자바 백엔드에서 가져온 값을 문자열로 변환
...
const 번호변경하기 = (key, value) => {
    ...
    if (key === 'phone'){
        return value.replace("-", ""); // '-' 를 제거 -를 "" 사용해서 없애기 적용
    }    
}
...
만약에 번호를 변경하기 않고, 기존에 가져온 유저정보를 그대로 사용할 경우에는
{JSON.stringfy(유저정보, null, 2)}
null = 변경사항없음을 사용하면 됨
*/