...
const Profile = () => {
  ...
  //2. axios async await 버전 = 3번의 업그레이드 버전 try/catch 사용해서 오류 처리
  ...
  const 이미지업로드3 = () => {
    ...
    //삼항연산자를 이용해서 수정기능을 위한 url과 새 프로필을 저장할 url 설정
    axios
      .post(uploadAPI, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
    ...
    <div>
      ...
      {/*
            <input type="file" multiple onChange={(e) => setFiles(e.target.files)} />
             ↑ ↓ 코드 동일한 기능을 작동하지만 const를 이용해서 기능을 구분짓는 것과 기능을 한 번에 작성해주는 차이
            <input type="file" value={files} multiple onChange={파일변경기능} />
       */}
