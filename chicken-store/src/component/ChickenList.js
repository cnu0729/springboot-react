      ...
      <ul>
        {chickens.map((chicken) => (
          <li key={chicken.id} className="chicken-item">
            ...
            <button className="detail-button" onClick={() => navigator(`/chicken-detail/${chicken.id}`)}>상세보기</button>
            
            {/*
            navigate와 Link 사용에 있어 태그를 사용하느냐, 기능을 사용하느냐의 차이
            큰 차이는 없음
            <button className="detail-button" onClick={() => navigator(`/chicken-detail/${chicken.id}`)}>상세보기</button>
            <Link to={`/chicken/${chicken.id}`}><button className="detail-button">상세보기</button></Link>
            */} 
            </li>
        ))}
      </ul>
